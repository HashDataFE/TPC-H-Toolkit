  ...
  read -u 5
  {
    ...
    if [ "${RUN_MODEL}" == "cloud" ]; then
      ...
      for file in ${GEN_DATA_PATH}/[0-9]*/${table_name}.tbl.[0-9]*; do
        if [ -e "$file" ]; then
          log_time "psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -c \"\COPY ${DB_SCHEMA_NAME}.${table_name} FROM 'sed "s/|$//" $file' WITH (FORMAT csv, DELIMITER '|', NULL '', ESCAPE E'\\\\\\\\', ENCODING 'LATIN1')\" | grep COPY | awk -F ' ' '{print \$2}'"
          result=$(
